...
class GitHubReleaseService {
  ...
  /// Queries GitHub Attestations API to verify whether [sha256Hash] has a valid
  /// SLSA provenance attestation generated by GitHub Actions for this repository.
  ///
  ...
  Future<bool> verifyArtifactAttestation(String sha256Hash) async {
    ...
    final uri = Uri.https(
      'api.github.com',
      '/repos/$repoOwner/$repoName/attestations/sha256:$cleanHash',
    );
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final jsonMap = json.decode(response.body) as Map<String, dynamic>;
        final attestations = jsonMap['attestations'] as List<dynamic>?;
        if (attestations != null && attestations.isNotEmpty) {
          developer.log(
            'SLSA provenance attestation successfully verified for hash: $cleanHash',
            name: 'GitHubReleaseService',
          );
          return true;
        }
      ...
      developer.log(
        'SLSA provenance attestation check failed (HTTP ${response.statusCode}) for hash: $cleanHash',
        name: 'GitHubReleaseService',
      );
      return false;
    } catch (e, stackTrace) {
      developer.log(
        'Error querying GitHub Attestations API: $e',
        name: 'GitHubReleaseService',
        error: e,
        stackTrace: stackTrace,
      );
