...
class SsrfValidator {
  ...
  static bool isBlockedIp(
    ...
    if (target.type == InternetAddressType.IPv6 && target.rawAddress.length == 16) {
      final b = target.rawAddress;
      final isMapped = b[0] == 0 && b[1] == 0 && b[2] == 0 && b[3] == 0 &&
    ...
    if (target.type == InternetAddressType.IPv4) {
      ...
      if (b0 == 169 && b1 == 254) return true; // Link-Local / Cloud Metadata (169.254.0.0/16)
    ...
    if (target.type == InternetAddressType.IPv6) {
      ...
      final isLinkLocalOrUla = target.isLinkLocal || ((bytes[0] & 0xfe) == 0xfc);
      if (isLinkLocalOrUla) {
        if (targetType == SsrfValidationTarget.webhook && allowLanWebhooks) {
