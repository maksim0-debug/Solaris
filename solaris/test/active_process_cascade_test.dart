...
void main() {
  ...
  setUpAll(() {
    ...
    const MethodChannel('com.solaris.monitor/names').setMockMethodCallHandler((MethodCall methodCall) async {
      return null;
    });
    const MethodChannel('flutter.baseflow.com/geolocator').setMockMethodCallHandler((MethodCall methodCall) async {
      return null;
    });
  });
  ...
  group('ActiveProcessService & Priority Cascades Zero-Trust Tests', () {
    ...
    group('ActiveProcessService Unit & State Machine Tests', () {
      ...
      test('updateActiveProcessManually updates activeProcess and windowTitle', () {
        final service = container.read(activeProcessServiceProvider.notifier);
        service.updateActiveProcessManually('photoshop.exe', title: 'Adobe Photoshop 2026');
      ...
      test('System shell processes are ignored and retain previous active process', () {
        final service = container.read(activeProcessServiceProvider.notifier);
        service.updateActiveProcessManually('photoshop.exe', title: 'Adobe Photoshop');

        for (final shellProc in ActiveProcessService.systemShellBlacklist) {
          service.updateActiveProcessManually(shellProc, title: 'Windows Shell');
          final state = container.read(activeProcessServiceProvider);
          expect(
            state.activeProcess,
            equals('photoshop.exe'),
            reason: 'Shell process $shellProc must not override active user process',
          );
        }
      });

      test('Immediate Preemption: switching directly between two profiled apps updates immediately (0 ms)', () {
        ...
        expect(container.read(activeProcessServiceProvider).activeProcess, equals('photoshop.exe'));

        // Direct switch to another profiled app -> Immediate Preemption
        service.updateActiveProcessManually('premiere.exe');
        expect(container.read(activeProcessServiceProvider).activeProcess, equals('premiere.exe'));
      });

      test('Suppressed PIDs can be set and are automatically cleared on switching user applications', () {
        final service = container.read(activeProcessServiceProvider.notifier);
        service.updateActiveProcessManually('photoshop.exe');

        service.suppressActiveApp(555);
        expect(container.read(activeProcessServiceProvider).suppressedPids, contains(555));

        // Switching to a new user app clears suppressedPids
        service.updateActiveProcessManually('blender.exe');
        expect(container.read(activeProcessServiceProvider).activeProcess, equals('blender.exe'));
        expect(container.read(activeProcessServiceProvider).suppressedPids, isEmpty);
      });
    });

    group('SettingsNotifier CRUD Methods Zero-Trust Tests', () {
      test('addAppOverride adds new rule and replaces existing rule with same exeName case-insensitively', () {
        ...
        final found = rules.firstWhere((r) => r.exeName == 'custom_editor.exe');
        ...
        final rules2 = container.read(settingsProvider).value?['all']?.appOverrides ?? [];
        final count = rules2.where((r) => r.exeName.toLowerCase() == 'custom_editor.exe').length;
        expect(count, equals(1));
        expect(rules2.firstWhere((r) => r.exeName.toLowerCase() == 'custom_editor.exe').fixedBrightness, equals(45.0));
      });
      ...
      test('updateAppOverride modifies existing rule in list', () {
        ...
        final modified = initial.copyWith(isEnabled: false, fixedTemperature: 5000.0, temperatureMode: AppOverrideMode.fixed);
        notifier.updateAppOverride(modified);

        final rules = container.read(settingsProvider).value?['all']?.appOverrides ?? [];
      ...
      test('toggleAppOverride enables and disables rule status', () {
        ...
        var rules = container.read(settingsProvider).value?['all']?.appOverrides ?? [];
        expect(rules.firstWhere((r) => r.exeName == 'toggle_me.exe').isEnabled, isFalse);

        notifier.toggleAppOverride('TOGGLE_ME.EXE', true);
        rules = container.read(settingsProvider).value?['all']?.appOverrides ?? [];
        expect(rules.firstWhere((r) => r.exeName == 'toggle_me.exe').isEnabled, isTrue);
      });

      test('resetBuiltInAppOverrides restores missing built-in rules without removing custom user rules', () {
        final notifier = container.read(settingsProvider.notifier);
      ...
      test('promoteBuiltInToUser changes isBuiltIn flag from true to false', () {
        final notifier = container.read(settingsProvider.notifier);

        notifier.promoteBuiltInToUser('photoshop.exe');
        final rules = container.read(settingsProvider).value?['all']?.appOverrides ?? [];
        final photoshopRule = rules.firstWhere((r) => r.exeName == 'photoshop.exe');
      ...
      test('updateAppOverrideExitDelay clamps values between 0 and 300', () {
        ...
        expect(container.read(settingsProvider).value?['all']?.appOverrideExitDelaySeconds, equals(150));

        notifier.updateAppOverrideExitDelay(-20);
        expect(container.read(settingsProvider).value?['all']?.appOverrideExitDelaySeconds, equals(0));

        notifier.updateAppOverrideExitDelay(500);
        expect(container.read(settingsProvider).value?['all']?.appOverrideExitDelaySeconds, equals(300));
      });
    });

    group('CurrentBrightnessNotifier Priority Cascade Zero-Trust Tests', () {
      test('Fixed Brightness App Override takes precedence over Circadian / Game / Manual', () {
        final settingsNotifier = container.read(settingsProvider.notifier);
        settingsNotifier.addAppOverride(
          const AppOverrideRule(
            exeName: 'photoshop.exe',
      ...
      test('Safe Fallback when curve preset ID is deleted/missing in brightness cascade', () {
        final settingsNotifier = container.read(settingsProvider.notifier);
        settingsNotifier.addAppOverride(
          const AppOverrideRule(
            exeName: 'blender.exe',
        ...
        expect(() => container.read(currentBrightnessProvider), returnsNormally);
      });

      test('Suppressed PIDs bypass App Override rule and fall back in cascade', () {
        final settingsNotifier = container.read(settingsProvider.notifier);
        settingsNotifier.addAppOverride(
          const AppOverrideRule(
            exeName: 'photoshop.exe',
      ...
      test('setManualBrightness calls suppressActiveApp and updates manual brightness', () {
        final activeProcessService = container.read(activeProcessServiceProvider.notifier);
        activeProcessService.updateActiveProcessManually('photoshop.exe');

        final brightnessNotifier = container.read(currentBrightnessProvider.notifier);
    ...
    group('CurrentTemperatureNotifier Priority Cascade Zero-Trust Tests', () {
      ...
      test('Fixed Temperature App Override takes precedence when temperature is enabled', () {
      ...
      test('Fixed Temperature is clamped strictly between 3300K and 6500K and rounded', () {
        container.read(isColorTemperatureEnabledProvider.notifier).set(true);
      ...
      test('Safe Fallback when curve preset ID is deleted/missing in temperature cascade', () {
        container.read(isColorTemperatureEnabledProvider.notifier).set(true);
        ...
        expect(() => container.read(currentTemperatureProvider), returnsNormally);
      });
      ...
      test('setManualTemperature calls suppressActiveApp and sets temperature', () {
        container.read(isColorTemperatureEnabledProvider.notifier).set(true);

        final activeProcessService = container.read(activeProcessServiceProvider.notifier);
        activeProcessService.updateActiveProcessManually('premiere.exe');

        final tempNotifier = container.read(currentTemperatureProvider.notifier);
