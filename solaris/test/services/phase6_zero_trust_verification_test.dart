...
void main() {
  ...
  group('Phase 6 - End-to-End GUI & Settings Integration Zero-Trust Verification Test Suite', () {
    ...
    test('1. Cascade GUI state change: Toggling isReadOnly = true in settingsProvider instantly blocks HTTP control & monitor mutations while keeping /health 100% public', () async {
      // Step A: Initially Full Access
      final initialReq = await client.postUrl(Uri.parse('$serverUrl/api/v1/control'));
      initialReq.headers.contentType = ContentType.json;
      initialReq.write(jsonEncode({'action': 'set_auto_brightness', 'enabled': true}));
      ...
      container.read(settingsProvider.notifier).updateApiPermissions(readOnlyConfig);
      await Future<void>.delayed(const Duration(milliseconds: 100));

      // Step C: Verify control action is blocked with 403 Forbidden
      final roReq = await client.postUrl(Uri.parse('$serverUrl/api/v1/control'));
      roReq.headers.contentType = ContentType.json;
      roReq.write(jsonEncode({'action': 'set_auto_brightness', 'enabled': true}));
      ...
      final monReq = await client.postUrl(Uri.parse('$serverUrl/api/v1/monitors/display-1/brightness'));
      ...
      final healthReq = await client.getUrl(Uri.parse('$serverUrl/api/v1/health'));
    ...
    test('2. Cascade GUI state change: Toggling read flag (allowReadWeather = false) in GUI immediately masks automation payload in GET /api/v1/status', () async {
      // Step A: Disable allowReadWeather in GUI
      const customConfig = ApiPermissionsConfig(allowReadWeather: false);
      container.read(settingsProvider.notifier).updateApiPermissions(customConfig);
      await Future<void>.delayed(const Duration(milliseconds: 100));
      ...
      final hasWeatherKeys = automation!.keys.any((k) => k.startsWith('weather_'));
      expect(hasWeatherKeys, isFalse);
    });

    test('3. Universal Privilege Escalation Guard: Remote payload attempting to inject apiPermissions is strictly rejected with 403 Forbidden', () async {
