...
void main() {
  ...
  group('ApiControlHandler & ApiMonitorsHandler Integration Tests', () {
    ...
    test(
      'POST /api/v1/control Batch Action fail_fast mode executes actions',
      () async {
    ...
    test(
      'POST /api/v1/control with unknown action returns 422 Unprocessable Entity',
      () async {
        final req = await client.postUrl(
          Uri.parse('$serverUrl/api/v1/control'),
        );
        req.headers.contentType = ContentType.json;
        req.write(jsonEncode({'action': 'unknown_action_xyz'}));
        final resp = await req.close();
    ...
    test(
      'POST /api/v1/monitors/display-1/brightness returns 202 Accepted',
      () async {
        final req = await client.postUrl(
