...
void main() {
  group('ApiPermissionsFilter Unit Tests', () {
    ...
    test('filterAutomation strips weather keys when allowReadWeather is false', () {
    ...
    test('filterAutomation strips circadian keys when allowReadCircadian is false', () {
    ...
    test('filterAutomation strips game_mode and offsets when allowReadMonitors is false', () {
    ...
    test('filterAutomation strips map_animations when system category is disabled', () {
    ...
    test('filterSmartCircadian removes sleep metrics when allowReadSleep is false', () {
      const config = ApiPermissionsConfig(
        allowReadCircadian: true,
        allowReadSleep: false,
      );
      final rawCircadian = sampleAutomation['smart_circadian'] as Map<String, dynamic>;
    ...
    test('filterSmartCircadian returns null when allowReadCircadian is false', () {
      const config = ApiPermissionsConfig(allowReadCircadian: false);
      final rawCircadian = sampleAutomation['smart_circadian'] as Map<String, dynamic>;
      final filtered = ApiPermissionsFilter.filterSmartCircadian(
        rawCircadian,
    ...
    test('filterPresets masks brightness/temperature based on monitor and circadian read flags', () {
      ...
      final res1 = ApiPermissionsFilter.filterPresets(samplePresets, noMonitorsConfig);
      expect(res1, isNotNull);
      expect(res1!.containsKey('brightness'), isFalse);
      expect(res1.containsKey('temperature'), isTrue);

      const noCircadianConfig = ApiPermissionsConfig(allowReadCircadian: false);
      final res2 = ApiPermissionsFilter.filterPresets(samplePresets, noCircadianConfig);
      ...
      final res3 = ApiPermissionsFilter.filterPresets(samplePresets, noneConfig);
      expect(res3, isNull);
    });
  });
}
