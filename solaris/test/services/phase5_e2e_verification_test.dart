...
void main() {
  ...
  group(
    'Phase 5 E2E Verification: WebSocket Engine & Windows Power Management',
    () {
      ...
      test('3. Selective Subscriptions Protocol (type: subscribe)', () async {
        ...
        final ack = await subAckCompleter.future.timeout(
          const Duration(seconds: 5),
        );
      ...
      test(
        '4. WebSocket Command Execution with Correlation ID (cmd_id)',
        () async {
          final wsUri = Uri.parse(
            'ws://127.0.0.1:$port/api/v1/ws?token=$authToken',
          );
      ...
      test(
        '5. CSWSH Guard & Untrusted Origin Rejection when Auth is required',
        () async {
      ...
      test('6. WindowsPowerListener Suspend and Resume Workflow', () async {
        final WindowsPowerListener listener = container.read(
      ...
      test(
        '7. WindowsPowerListener Hardware Error & Display Change Events',
        () async {
          final WindowsPowerListener listener = container.read(
            windowsPowerListenerProvider,
          );
          ...
          expect(
            () => listener.handleHardwareError('I2C Bus NACK Timeout'),
            returnsNormally,
          );
      ...
      test('8. WebhookEventType enum includes Phase 5 system events', () {
        expect(
          WebhookEventType.onSystemResume.wireName,
          equals('on_system_resume'),
        );
        expect(
          WebhookEventType.onHardwareError.wireName,
          equals('on_hardware_error'),
        );
      });
    },
  );
