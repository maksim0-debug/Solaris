...
void main() {
  ...
  group('Phase 2 E2E Verification & Comprehensive 26 Actions Test Suite', () {
    ...
    Future<HttpClientResponse> sendPost(
      String path,
    ...
    test(
      'Verification: Action 2 & 3 - set_auto_brightness & toggle_auto_brightness',
      () async {
    ...
    test(
      'Verification: Action 5, 6, 7 - Temperature toggles & auto-mode',
      () async {
        var resp = await sendPost('/api/v1/control', {
          'action': 'set_color_temperature_enabled',
          'enabled': true,
        });
        expect(resp.statusCode, equals(HttpStatus.ok));

        resp = await sendPost('/api/v1/control', {
          'action': 'set_auto_temperature',
          'enabled': true,
        });
        expect(resp.statusCode, equals(HttpStatus.ok));
    ...
    test(
      'Verification: Action 8, 9, 10, 11 - Presets (brightness, temperature, user, cycle)',
      () async {
        var resp = await sendPost('/api/v1/control', {
    ...
    test('Verification: Action 12, 13, 14 - Game Mode & Whitelist', () async {
      ...
      resp = await sendPost('/api/v1/control', {
        'action': 'set_game_mode_brightness',
        'value': 90.0,
      });
      expect(resp.statusCode, equals(HttpStatus.accepted));

      resp = await sendPost('/api/v1/control', {
        'action': 'manage_game_mode_whitelist',
        'op': 'add',
        'app': 'cyberpunk2077.exe',
      });
      expect(resp.statusCode, equals(HttpStatus.ok));
    ...
    test(
      'Verification: Action 16, 17, 18 - Weather Adjustments & Intensity',
      () async {
        var resp = await sendPost('/api/v1/control', {
          'action': 'set_weather_adjustment',
          'brightness': true,
          'temperature': false,
        });
        expect(resp.statusCode, equals(HttpStatus.ok));
        ...
        resp = await sendPost('/api/v1/control', {
          'action': 'set_weather_temperature_adjustment',
          'enabled': true,
        });
        expect(resp.statusCode, equals(HttpStatus.ok));

        resp = await sendPost('/api/v1/control', {
          'action': 'set_weather_intensity',
          'value': 0.7,
        });
        expect(resp.statusCode, equals(HttpStatus.accepted));
      },
    ...
    test(
      'Verification: Action 19 & 20 - Smart Circadian Master & Submodules',
      () async {
        var resp = await sendPost('/api/v1/control', {
          'action': 'set_smart_circadian',
          'enabled': true,
        });
        expect(resp.statusCode, equals(HttpStatus.ok));
    ...
    test(
      'Verification: Action 21, 22, 23, 24 - Map Animations, Location, Provider, Sun Sync',
      () async {
        var resp = await sendPost('/api/v1/control', {
    ...
    test(
      'Verification: Action 25 & 26 - Sleep status push & Clear failed webhooks',
      () async {
        var resp = await sendPost('/api/v1/control', {
    ...
    test(
      'Verification: Per-Monitor endpoints and 404 for unknown slug',
      () async {
        ...
        resp = await sendPost('/api/v1/monitors/display-1/brightness', {
          'value': 40.0,
        });
        expect(resp.statusCode, equals(HttpStatus.accepted));

        resp = await sendPost('/api/v1/monitors/display-1/temperature', {
          'value': 4000,
        });
        expect(resp.statusCode, equals(HttpStatus.accepted));
      },
    ...
    test(
      'Verification: Fail-Fast Batch execution stops on invalid action',
      () async {
        final resp = await sendPost('/api/v1/control', {
          'mode': 'fail_fast',
          'actions': [
