...
void main() {
  ...
  group('Phase 2 Zero-Trust Exhaustive Audit: BDD Behavioral Specifications', () {
    ...
    test('Scenario 1: Given allowedActions containing only set_brightness, When set_temperature is requested, Then executeAction rejects with 403 Forbidden details', () async {
    ...
    test('Scenario 2: Given allowedActions containing set_brightness_preset, When preset shorthand alias "brightest" is sent without preset payload key, Then parameter preset="brightest" is injected and action succeeds', () async {
      const perms = ApiPermissionsConfig(
    ...
    test('Scenario 3: Given allowedActions containing set_weather_provider, When provider alias "openmeteo" is requested, Then provider="openmeteo" is injected into payload', () async {
      const perms = ApiPermissionsConfig(
    ...
    test('Scenario 4: Given allowedActions with clear_failed_webhooks alias, When executeAction is called, Then it routes to combined manage_webhooks / clear_failed_webhooks branch seamlessly', () async {
      const perms = ApiPermissionsConfig(
        allowedActions: {'manage_webhooks'},
      );

      final result = await controlHandler.executeAction(
        {'action': 'clear_failed_webhooks', 'webhook_id': 'wh_test_1'},
        permissions: perms,
      );
    ...
    test('Scenario 5: Given internal-only action "on_system_resume", When requested via REST executeAction, Then it returns 422 Unprocessable Entity', () async {
  ...
  group('Phase 2 Zero-Trust Exhaustive Audit: Property-Based Testing (PBT)', () {
    test('PBT Invariant 1: For any random subset S of canonical actions, checkAction(a) is allowed iff canonical(a) in S', () {
      final random = Random(42);
      final allCanonical = ApiPermissionsConfig.getAllCanonicalActions().toList();
      final allAliases = [
      ...
      for (int i = 0; i < 100; i++) {
        ...
        for (final alias in allAliases) {
          ...
          expect(checkRes.isAllowed, equals(selectedSubset.contains(canonical)));
        }
      }
    });

    test('PBT Invariant 2: Action is NEVER allowed if parent category is disabled, even if action is explicitly in allowedActions', () {
      ...
      for (int i = 0; i < 50; i++) {
        ...
        final disabledCategory = allCategories[random.nextInt(allCategories.length)];
        final enabledCategories = allCategories.where((c) => c != disabledCategory).toSet();
        ...
        final categoryActions = ApiPermissionsConfig.getActionsForCategory(disabledCategory);
  ...
  group('Phase 2 Zero-Trust Exhaustive Audit: Real HTTP REST Server Integration', () {
    ...
    setUp(() async {
      ...
      router.post('/api/v1/control', (HttpRequest req, Map<String, String> params) async {
        final rawPermsHeader = req.headers.value('X-Test-Permissions');
        final perms = rawPermsHeader != null
            ? ApiPermissionsConfig.fromJson(jsonDecode(rawPermsHeader) as Map<String, dynamic>)
            : const ApiPermissionsConfig();
        final payload = jsonDecode(await utf8.decoder.bind(req).join()) as Map<String, dynamic>;
        await controlHandler.handleControlWithPermissions(req, payload, perms);
      });

      router.post('/api/v1/monitors/:slug/brightness', (HttpRequest req, Map<String, String> params) async {
        final rawPermsHeader = req.headers.value('X-Test-Permissions');
        final perms = rawPermsHeader != null
            ? ApiPermissionsConfig.fromJson(jsonDecode(rawPermsHeader) as Map<String, dynamic>)
            : const ApiPermissionsConfig();
        await monitorsHandler.handleSetMonitorBrightnessWithPermissions(req, params, perms);
      });
    ...
    test('HTTP REST POST /api/v1/control returns 403 RFC7807 when action is prohibited by allowedActions', () async {
      final req = await client.postUrl(Uri.parse('$serverUrl/api/v1/control'));
      req.headers.contentType = ContentType.json;
      req.headers.set(
        'X-Test-Permissions',
        jsonEncode(const ApiPermissionsConfig(allowedActions: {'set_brightness'}).toJson()),
      );
    ...
    test('HTTP REST POST /api/v1/monitors/primary/brightness returns 403 RFC7807 when set_brightness is prohibited', () async {
      final req = await client.postUrl(Uri.parse('$serverUrl/api/v1/monitors/primary/brightness'));
      req.headers.contentType = ContentType.json;
      req.headers.set(
        'X-Test-Permissions',
        jsonEncode(const ApiPermissionsConfig(allowedActions: {'set_temperature'}).toJson()),
      );
    ...
    test('HTTP REST Batch Mode fail_fast Pre-flight aborts batch when prohibited alias "brightest" is present in actions list', () async {
      final req = await client.postUrl(Uri.parse('$serverUrl/api/v1/control'));
      req.headers.contentType = ContentType.json;
      req.headers.set(
        'X-Test-Permissions',
        jsonEncode(const ApiPermissionsConfig(allowedActions: {'set_auto_brightness'}).toJson()),
      );
      req.write(jsonEncode({
        'mode': 'fail_fast',
        'actions': [
          {'action': 'set_auto_brightness', 'enabled': true},
          {'action': 'brightest'}, // Prohibited alias (normalizes to set_brightness_preset)
        ],
      }));
      final resp = await req.close();
  ...
  group('Phase 2 Zero-Trust Exhaustive Audit: WebSocket Command ACL & Mutation Events Isolation', () {
    test('WebSocket broadcastEvent filters internal mutation events by client allowedActions', () {
      const permsWithResume = ApiPermissionsConfig(allowedActions: {'on_system_resume'});
      const permsWithoutResume = ApiPermissionsConfig(allowedActions: {'set_brightness'});

      expect(permsWithResume.isActionAllowed('on_system_resume'), isTrue);
      expect(permsWithoutResume.isActionAllowed('on_system_resume'), isFalse);
    });
  });
}
...
extension on ApiControlHandler {
  Future<void> handleControlWithPermissions(
    ...
    if (actions is List) {
      await handleBatchControlWithPermissions(request, jsonPayload, permissions);
    } else {
      final result = await executeAction(jsonPayload, permissions: permissions);
      final statusCode = result['status'] == 'error'
          ? (result['error'] == 'unprocessable_entity' ? HttpStatus.unprocessableEntity : HttpStatus.forbidden)
          : (result['status'] == 'accepted' ? HttpStatus.accepted : HttpStatus.ok);

      if (result['status'] == 'error') {
        final title = result['error'] == 'action_prohibited' ? 'Action Prohibited' : 'Control Error';
