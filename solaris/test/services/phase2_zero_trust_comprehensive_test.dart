...
void main() {
  ...
  group(
    'Phase 2 - Scoped API Keys, Middleware & Routing Engine Comprehensive Zero-Trust Verification',
    () {
      ...
      setUp(() async {
        ...
        final globalState =
            container.read(settingsProvider).value?['all'] ?? SettingsState();
      ...
      group('1. Constant-Time Lookup & Key Matching', () {
        test(
          'findMatchingKey accurately resolves valid tokens and rejects invalid tokens',
          () {
        ...
        test(
          'Single Key Protection Guard prevents deleting the last remaining API key',
          () {
            ...
            expect(
              ...
              isFalse,
              reason:
                  'Single Key Guard must prevent deleting the sole remaining key',
            );

            final updatedKeys = container
                .read(settingsProvider)
                .value!['all']!
                .apiKeys;
      ...
      group('2. HTTP Auth & CORS Middleware Rules', () {
        test(
          'OPTIONS CORS preflight bypasses auth check with 200 OK or 204 No Content',
          () async {
        ...
        test(
          'Invalid token on loopback is rejected with 401 Unauthorized',
          () async {
        ...
        test(
          'Valid Scoped Key enforces permissions on mutating endpoints (403 when Read-Only)',
          () async {
        ...
        test(
          'Require Local Token setting blocks tokenless loopback calls',
          () async {
            container
                .read(settingsProvider.notifier)
                .updateRequireLocalToken(true);
        ...
        test(
          'OpenAPI spec endpoint adapts dynamically to requested API Key permissions',
          () async {
            ...
            final monitorGet =
                (paths['/api/v1/monitors']
                        as Map<String, dynamic>)['get']['responses']
                    as Map<String, dynamic>;
            expect(
              monitorGet.containsKey('403'),
              isTrue,
              reason: 'Disabled category must specify 403 Forbidden response',
            );
          },
        );
      });
      ...
      group('3. Privilege Escalation Protection', () {
        test(
          'POST /api/v1/control rejects payloads containing privilege escalation keys',
          () async {
        ...
        test(
          'Batch action payload with nested escalation key is recursively blocked',
          () async {
            final validKey = ApiKeyEntry.create(name: 'Valid Key');
            container.read(settingsProvider.notifier).addApiKey(validKey);
            router.apiKeys = container
      ...
      group('4. WebSocket Isolation & Reactive Revocation', () {
        test(
          'WebSocket upgrade with invalid token is immediately rejected with 401',
          () async {
            expect(
        ...
        test(
          ...
          () async {
            ...
            final jsonMap =
                jsonDecode(firstMsg as String) as Map<String, dynamic>;
