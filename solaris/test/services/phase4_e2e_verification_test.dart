...
void main() {
  ...
  group('Phase 4: Comprehensive Zero-Trust E2E & Verification Suite', () {
    ...
    group('1. SSRF Validator Security Boundary Checks', () {
      ...
      test(
        'Cloud Metadata / Link-Local IPv4 (169.254.0.0/16) is strictly blocked',
        () {
          final metadataIps = [
            '169.254.169.254',
            '169.254.0.1',
      ...
      test('IPv4-mapped IPv6 addresses are correctly unmapped and validated', () {
        ...
        expect(
          ...
          isTrue,
          reason:
              'Mapped IPv6 loopback ::ffff:127.0.0.1 must be normalized to 127.0.0.1 and blocked',
        );
      ...
      test('Multicast and reserved IPs are blocked', () {
        ...
        expect(
          SsrfValidator.isBlockedIp(InternetAddress('239.255.255.250')),
          isTrue,
        );
      ...
      test(
        'Private LAN IPv4 & IPv6 Link-Local/ULA behavior with allowLanWebhooks flag',
        () {
          final lanIps = [
            '192.168.1.100',
            '10.0.0.1',
          ...
          for (final ipStr in lanIps) {
            ...
            expect(
              ...
              isFalse,
              reason:
                  'LAN IP $ipStr should be allowed for Home Assistant / Node-RED in LAN webhook mode',
            );
            ...
            expect(
              ...
              isTrue,
              reason:
                  'LAN IP $ipStr must be blocked when allowLanWebhooks is false',
            );
          }
        },
      );
    ...
    group('2. WebhookEventType Enum Complete Events Verification', () {
      test(
        'All WebhookEventType values are defined and have non-empty wireNames',
        () {
          final events = WebhookEventType.values;
          expect(
            events.length,
            equals(WebhookEventType.values.length),
            reason: 'All distinct event types must have non-empty wireNames',
          );
      ...
      test(
        'WebhookEventType.fromString converts both wireName and enum name correctly',
        () {
    ...
    group('3. WebhookConfig HMAC Signature & DPAPI Obfuscation', () {
      test('HMAC-SHA256 signature matches expected pattern', () {
        ...
        expect(
          signature.length,
          equals(7 + 64),
          reason: 'sha256= prefix (7 chars) + 64 hex chars = 71 chars',
        );
      });
      ...
      test('HMAC-SHA256 returns null if secretKey is null or empty', () {
        ...
        expect(
          configNoSecret.calculateSignature('deliv-1', 'ts', 'body'),
          isNull,
        );

        final configEmptySecret = configNoSecret.copyWith(secretKey: '');
        expect(
          configEmptySecret.calculateSignature('deliv-1', 'ts', 'body'),
          isNull,
        );
      });

      test(
        'WebhookConfig JSON round-trip encrypts and decrypts secretKey & customHeaders',
        () {
          final config = WebhookConfig(
            id: 'wh_test_dpapi',
            ...
            name: 'N8N Automation',
            events: {
              WebhookEventType.onGameModeActivated,
              WebhookEventType.onGameModeDeactivated,
            },
            isEnabled: true,
          ...
          expect(
            ...
            isNot(equals('Bearer token_12345')),
          );
          ...
          expect(
            restored.customHeaders?['Authorization'],
            equals('Bearer token_12345'),
          );
          expect(
            restored.customHeaders?['X-Custom-Header'],
            equals('CustomValue'),
          );
        },
      );
      ...
      test(
        'WebhookConfig.copyWith handles clearSecretKey and clearCustomHeaders',
        () {
          final config = WebhookConfig(
            id: 'wh_copy',
    ...
    group('4. SsrfSafeHttpClient Security Exception on Blocked IPs', () {
      test(
        'SsrfSafeHttpClient throws SecurityException when attempting to request loopback 127.0.0.1',
        () async {
          ...
          expect(
            ...
            reason:
                'Requesting 127.0.0.1 via SsrfSafeHttpClient with allowLanWebhooks=false must trigger SecurityException',
          );
        },
      );
    });
    ...
    group('5. WAL Crash Recovery & Transaction Serialization', () {
      ...
      test(
        'WAL file recovery restores .tmp file if main file is missing',
        () async {
          ...
          try {
            ...
            await tmpFile.writeAsString(
              '{"deliveryId":"d1","webhookId":"w1","url":"http://x","eventName":"e1","payloadJson":"{}"}\n',
            );
    ...
    group('6. Privacy-First Data Sanitization Verification', () {
      test(
        'onWeatherUpdated sanitization removes GPS coordinates and city name',
        () {
    ...
    group('7. Phase 4 Webhook REST API Endpoints Integration', () {
      ...
      setUp(() async {
        ...
        final localIpcService = container.read(
      ...
      test('GET /api/v1/webhooks returns webhooks list structure', () async {
        final req = await client.getUrl(
          Uri.parse('$serverUrl/api/v1/webhooks'),
        );
      ...
      test(
        'GET /api/v1/webhooks/events returns all available events',
        () async {
          final req = await client.getUrl(
            Uri.parse('$serverUrl/api/v1/webhooks/events'),
          );
          final res = await req.close();
          expect(res.statusCode, equals(HttpStatus.ok));
      ...
      test(
        'POST /api/v1/webhooks creates a new webhook configuration',
        () async {
          ...
          expect(
            json['webhook']['url'],
            equals('https://homeassistant.local:8123/api/webhook/solaris'),
          );
        },
      );

      test(
        'POST /api/v1/webhooks rejects invalid or relative URLs with 400 Bad Request',
        () async {
          final req = await client.postUrl(
            Uri.parse('$serverUrl/api/v1/webhooks'),
          );
          req.headers.contentType = ContentType.json;
          req.write(jsonEncode({'url': 'invalid-not-a-url'}));
      ...
      test('DELETE /api/v1/webhooks/:id deletes webhook by ID', () async {
        final req = await client.deleteUrl(
          Uri.parse('$serverUrl/api/v1/webhooks/wh_integration_test_1'),
        );
        req.headers.contentType = ContentType.json;
        final res = await req.close();
        expect(res.statusCode, equals(HttpStatus.ok));
      ...
      test(
        'GET /api/v1/webhooks/dlq and POST /api/v1/webhooks/dlq/retry behave cleanly',
        () async {
          // GET DLQ
          final getReq = await client.getUrl(
            Uri.parse('$serverUrl/api/v1/webhooks/dlq'),
          );
          ...
          final postReq = await client.postUrl(
            Uri.parse('$serverUrl/api/v1/webhooks/dlq/retry'),
          );
