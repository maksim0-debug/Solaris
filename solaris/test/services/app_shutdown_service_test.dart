...
void main() {
  ...
  group('AppShutdownService Tests', () {
    test(
      'performShutdown executes steps without throwing exceptions',
      () async {
    ...
    test(
      'appShutdownServiceProvider instantiates AppShutdownService correctly',
      () {
        final container = ProviderContainer();
        addTearDown(container.dispose);

        final shutdownService = container.read(
          appShutdownServiceProvider(container),
        );
