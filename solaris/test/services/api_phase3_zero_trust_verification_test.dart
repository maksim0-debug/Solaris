...
void main() {
  ...
  group('Phase 3 - Zero-Trust Verification & Complete QA Test Suite', () {
    ...
    group('A. BDD Behavioral Scenarios — Control & Mutation ACL', () {
      test(
        'Scenario 1: Read-Only mode blocks single control action with RFC 7807',
        () async {
      ...
      test(
        'Scenario 2: Privilege Escalation Guard rejects apiPermissions key in payload',
        () async {
      ...
      test(
        'Scenario 3: Disabled category blocks single control action',
        () async {
          updatePermissions(
      ...
      test(
        'Scenario 4: Pre-flight ACL check rejects ENTIRE batch in fail_fast mode before any mutations',
        () async {
          updatePermissions(
            const ApiPermissionsConfig(
              allowedCategories: {
      ...
      test(
        'Scenario 5: Batch in continue mode marks prohibited action with 403 error status and executes allowed ones',
        () async {
          updatePermissions(
            const ApiPermissionsConfig(
      ...
      test(
        'Scenario 6: POST /api/v1/monitors/:slug/brightness is blocked in Read-Only mode',
        () async {
          updatePermissions(const ApiPermissionsConfig(isReadOnly: true));
      ...
      test(
        'Scenario 7: POST /api/v1/monitors/:slug/temperature is blocked when monitors category is disabled',
        () async {
          updatePermissions(
            const ApiPermissionsConfig(
    ...
    group('B. Property-Based Testing (PBT) — Invariants Verification', () {
      test(
        'PBT Invariant 1: When isReadOnly is true, isActionAllowed returns false for ALL actions',
        () {
          ...
          for (final action in testActions) {
            expect(
              ...
              isFalse,
              reason: 'Action "$action" must be disallowed in Read-Only mode',
            );
          }
        },
      );
      ...
      test(
        'PBT Invariant 2: getCategoryForAction correctly maps all 30+ supported action strings and aliases',
        () {
          final mappings = {
          ...
          mappings.forEach((action, expectedCategory) {
            ...
            expect(
              ...
              reason: 'Action "$action" should map to ${expectedCategory.name}',
            );
          });
        },
      );
