...
void main() {
  ...
  group('Stage 2: ApiPermissionsChecker.checkAction Tests', () {
    test('checkAction denies when isReadOnly is true', () {
      ...
      final result = ApiPermissionsChecker.checkAction(config, 'set_brightness');
    ...
    test('checkAction denies unknown action with 422 error details', () {
      ...
      final result = ApiPermissionsChecker.checkAction(config, 'invalid_action_xyz');
    ...
    test('checkAction allows action when category is enabled and allowedActions is null', () {
      const config = ApiPermissionsConfig();
      final result = ApiPermissionsChecker.checkAction(config, 'set_brightness');
  ...
  group('Stage 2: ApiControlHandler Per-Action ACL & Alias Parameters Injection', () {
    ...
    test('executeAction denies action excluded from allowedActions', () async {
    ...
    test('executeAction permits alias and injects parameter into payload', () async {
      const perms = ApiPermissionsConfig(
        allowedActions: {'set_brightness_preset'},
      );
    ...
    test('executeAction permits toggle alias without requiring enabled boolean', () async {
      const perms = ApiPermissionsConfig(
    ...
    test('executeAction permits webhook alias clear_failed_webhooks via manage_webhooks permission', () async {
      const perms = ApiPermissionsConfig(
        allowedActions: {'manage_webhooks'},
      );

      final result = await handler.executeAction(
        {'action': 'clear_failed_webhooks', 'webhook_id': 'wh_123'},
        permissions: perms,
      );
  ...
  group('Stage 2: OpenApiSpec allowedActions Reflection', () {
    test('generateSpec includes AllowedActions in info description when active', () {
      const perms = ApiPermissionsConfig(
      ...
      expect(description, contains('AllowedActions=[set_brightness, set_temperature]'));
    });
  });
}
