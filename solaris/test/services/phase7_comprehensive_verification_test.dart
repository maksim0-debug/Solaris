...
void main() {
  group('Phase 7 Comprehensive Verification & Final QA Suite', () {
    group('7.A BDD / Gherkin Specification Scenarios', () {
      test(
        'Scenario 1: Block mutation requests when Read-Only mode is active',
        () {
      ...
      test(
        'Scenario 2: Prevent Privilege Escalation via API mutation payload',
        () {
      ...
      test(
        'Scenario 3: Granular masking of automation composite object in WebSocket updates',
        () {
          const config = ApiPermissionsConfig(
      ...
      test(
        'Scenario 4: Sub-filtering of smart_circadian object when allowReadSleep is false',
        () {
          const config = ApiPermissionsConfig(
            allowReadCircadian: true,
      ...
      test(
        'Scenario 5: Pre-flight ACL check rejects entire batch in fail_fast mode',
        () {
          const config = ApiPermissionsConfig(
      ...
      test('Scenario 6: Unrestricted access to health check endpoint', () {
        ...
        final healthSpec = OpenApiSpec.generateSpec(
          permissions: config,
    ...
    group('7.B Property-Based Testing (PBT Invariants & Combinatorial Grid)', () {
      test(
        'Invariants for filterAutomation: Weather keys strictly excluded when allowReadWeather = false across 32 combinations',
        () {
          final weatherKeys = [
          ...
          for (int i = 0; i < 32; i++) {
            ...
            final result = ApiPermissionsFilter.filterAutomation(
              sampleRaw,
              config,
            );
            for (final wKey in weatherKeys) {
              expect(
                result.containsKey(wKey),
                isFalse,
                reason: 'Combination $i leaked $wKey!',
              );
            }
          }
        },
      ...
      test(
        'Invariants for isActionAllowed: All 30+ actions return false when isReadOnly = true',
        () {
          const config = ApiPermissionsConfig(isReadOnly: true);
          ...
          for (final action in testActions) {
            expect(
              ...
              isFalse,
              reason: 'Action $action was allowed in Read-Only!',
            );
          }
        },
      );
    ...
    group('7.C OpenAPI 3.0.3 Spec Dynamic Permissions Integration', () {
      test(
        'generateSpec populates 403 responses and Read-Only annotations when permissions is provided',
        () {
          const config = ApiPermissionsConfig(
            isReadOnly: true,
            allowReadSolar: false,
          ...
          expect(
            ...
            contains('[Permissions Active]: ReadOnly=true'),
          );
          ...
          expect(
            paths['/api/v1/health']['get']['responses'].containsKey('403'),
            isFalse,
          );

          // Check 403 on restricted read paths
          expect(
            paths['/api/v1/solar']['get']['responses'].containsKey('403'),
            isTrue,
          );
          expect(
            paths['/api/v1/sleep/sessions']['get']['responses'].containsKey(
              '403',
            ),
            isTrue,
          );
      ...
      test(
        'generateJsonSpec returns valid JSON string with permissions active',
        () {
          const config = ApiPermissionsConfig();
          final jsonString = OpenApiSpec.generateJsonSpec(
            port: 45321,
            permissions: config,
          );
