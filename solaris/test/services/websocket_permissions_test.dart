...
class TestSettingsNotifier extends SettingsNotifier {
  ...
  void updatePermissions(ApiPermissionsConfig newPermissions) {
    state = AsyncData({
      'all': state.value!['all']!.copyWith(apiPermissions: newPermissions),
    });
  }
}
...
void main() {
  ...
  test(
    'Phase 2 WebSocket: Initial Snapshot filtering according to API permissions',
    () async {
      ...
      final wsUri = Uri.parse(
        'ws://127.0.0.1:$port/api/v1/ws?token=test-ws-token-123',
      );
      ...
      final snapshotMsg = await snapshotCompleter.future.timeout(
  ...
  test(
    'Phase 2 WebSocket: Selective subscription denial for disabled read flags',
    () async {
      final customPermissions = const ApiPermissionsConfig(
        allowReadSleep: false,
        allowReadMonitors: true,
      ...
      final deniedMsg = await deniedCompleter.future.timeout(
        const Duration(seconds: 5),
      );
      expect(deniedMsg['type'], equals('subscription_denied'));
      expect(deniedMsg['module'], equals('sleep'));

      final subMsg = await subscribedCompleter.future.timeout(
        const Duration(seconds: 5),
      );
  ...
  test(
    'Phase 2 WebSocket: Runtime ACL subscription audit notifies client with subscription_revoked',
    () async {
      ...
      notifier.updatePermissions(
        const ApiPermissionsConfig(allowReadSleep: false),
      );

      final revokedMsg = await revokedCompleter.future.timeout(
        const Duration(seconds: 5),
      );
  ...
  test(
    'Phase 2 WebSocket: Command privilege escalation attempt is rejected',
    () async {
      final initialSettings = SettingsState(
        isLocalIpcServerEnabled: true,
        apiAccessToken: 'test-ws-token-123',
      ...
      final resp = await responseCompleter.future.timeout(
        const Duration(seconds: 5),
      );
  ...
  test(
    'Phase 2 WebSocket: Complete isolation when all read permissions are disabled',
    () async {
  ...
  test(
    'Phase 2 WebSocket: Dynamic broadcast filtering masks smart_circadian sleep pressure and debt',
    () async {
        ...
        if (message is String) {
          ...
          if (jsonMap['type'] == 'snapshot') {
            snapshotCompleter.complete();
              ...
              jsonMap['module'] == 'smart_circadian') {
      ...
      final updateMsg = await updateCompleter.future.timeout(
        const Duration(seconds: 5),
      );
  ...
  test(
    'Phase 2 WebSocket: Event broadcast rejected when category is disabled',
    () async {
      final permissions = const ApiPermissionsConfig(
        allowedCategories: {
          ApiActionCategory.monitors,
        }, // system category omitted
      );
  ...
  test(
    'Phase 2 WebSocket: Command ACL rejects action when category is not allowed or read-only',
    () async {
      final permissions = const ApiPermissionsConfig(
        isReadOnly: false,
        allowedCategories: {
          ApiActionCategory.monitors,
        }, // sleep category omitted
      );
  ...
  test(
    'Phase 2 WebSocket: Zero Memory Leak & Client Disconnect Cleanup',
    () async {
      final initialSettings = SettingsState(
        isLocalIpcServerEnabled: true,
        apiAccessToken: 'test-ws-token-123',
