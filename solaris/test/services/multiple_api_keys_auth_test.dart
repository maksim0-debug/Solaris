...
void main() {
  group('Stage 2: Multiple Scoped API Keys Engine & Routing Tests', () {
    ...
    test('ApiKeyEntry matching via constant-time SHA-256 (findMatchingKey)', () {
      ...
      final matchedInvalid = router.findMatchingKey('sol_sec_invalidtoken12345');
    ...
    test('Legacy expectedToken proxy property backward compatibility', () {
      ...
      router.expectedToken = 'sol_sec_newlegacytoken1234567890abcdef1234567890abcdef12345678';
      expect(router.apiKeys.first.token, equals('sol_sec_newlegacytoken1234567890abcdef1234567890abcdef12345678'));
      expect(router.expectedToken, equals('sol_sec_newlegacytoken1234567890abcdef1234567890abcdef12345678'));
    });

    test('OpenAPI spec dynamic filtering according to key permissions', () {
      final fullSpec = OpenApiSpec.generateSpec(port: 45321, permissions: fullKey.permissions);
      expect(fullSpec, isA<Map<String, dynamic>>());

      final readOnlySpec = OpenApiSpec.generateSpec(port: 45321, permissions: readOnlyKey.permissions);
      expect(readOnlySpec, isA<Map<String, dynamic>>());
    });

    test('HTTP Auth Middleware attaches permissions and enforces invalid token 401', () async {
      ...
      final reqInvalid = await client.getUrl(Uri.parse('http://127.0.0.1:${server!.port}/api/v1/test'));
      ...
      final reqRO = await client.getUrl(Uri.parse('http://127.0.0.1:${server!.port}/api/v1/test'));
      reqRO.headers.add('x-api-key', readOnlyKey.token);
      final resRO = await reqRO.close();
      expect(resRO.statusCode, equals(HttpStatus.ok));
      final jsonRO = jsonDecode(await utf8.decoder.bind(resRO).join()) as Map<String, dynamic>;
    ...
    test('HTTP Require Local Token setting rejects tokenless loopback requests', () async {
      ...
      final reqAnon = await client.getUrl(Uri.parse('http://127.0.0.1:${server!.port}/api/v1/test'));
      ...
      final reqValid = await client.getUrl(Uri.parse('http://127.0.0.1:${server!.port}/api/v1/test'));
