...
void main() {
  group('Phase 3 Comprehensive Verification & Zero-Trust Test Suite', () {
    ...
    group('1. BDD Behavioral Specifications (Given / When / Then)', () {
      test(
        'Scenario 1: Given Localhost CLI request without Origin/Referer, When LAN is disabled, Then access is granted without token',
        () async {
          // Given
          router.requireLocalToken = false;
          router.isLanEnabled = false;
          ...
          expect(
            constantTimeEquals(
              'sol_sec_key_99887766554433221100aabbccdd',
      ...
      test(
        'Scenario 2: Given external browser drive-by request with Origin header, When token is missing, Then access is denied with 403',
        () async {
          ...
          final isUntrusted =
              !isLocalHostUri(origin) || !isLocalHostUri(referer);
          expect(
            isUntrusted,
            isTrue,
            reason: 'External browser origin must be flagged as untrusted',
          );
        },
      );

      test(
        'Scenario 3: Given LAN access enabled, When request contains valid X-API-Key, Then access is granted',
        () async {
          // Given
      ...
      test(
        'Scenario 4: Given LAN access enabled, When invalid X-API-Key is provided, Then authentication fails',
        () async {
          // Given
          router.isLanEnabled = true;
          ...
          // When
          final isValid = constantTimeEquals(
            invalidToken,
            router.expectedToken,
          );
    ...
    group('2. Property-Based Testing (PBT) — Token Invariants & Edge Cases', () {
      ...
      test(
        'PBT Invariant: constantTimeEquals always returns true iff input exactly matches expected',
        () {
          ...
          for (int i = 0; i < 100; i++) {
            ...
            if (randomStr == expected) {
              ...
              expect(
                ...
                isFalse,
                reason:
                    'Random string "$randomStr" should not match "$expected"',
              );
            }
          }
        },
      ...
      test(
        'PBT Invariant: Non-Empty Token Guard strictly rejects empty or whitespace inputs',
        () {
      ...
      test(
        'PBT Invariant: constantTimeEquals handles huge inputs (>10,000 chars) without crashing or memory leaks',
        () {
          const expected = 'sol_valid_token_777';
      ...
      test(
        'PBT Invariant: constantTimeEquals handles unicode, emoji and multi-byte utf8 chars cleanly',
        () {
          const expected = 'sol_token_🔐_unicode_123';
          expect(
            constantTimeEquals('sol_token_🔐_unicode_123', expected),
            isTrue,
          );
          expect(
            constantTimeEquals('sol_token_🔓_unicode_123', expected),
          ...
          expect(
            constantTimeEquals('sol_token_🔐_unicode_124', expected),
            isFalse,
          );
        },
      );
    ...
    group('3. Middleware & Security Infrastructure Unit Tests', () {
      test(
        'Host Header extraction handles IPv4, IPv6 brackets, ports and multi-homed hosts',
        () {
    ...
    group('4. Windows Firewall Service Deep Verification', () {
      ...
      test(
        'getRuleName formats name strictly matching rulePrefix for clean deletion',
        () {
          expect(
          ...
          expect(
            WindowsFirewallService.getRuleName(8080),
            equals('Solaris_Control_API_Port_8080'),
          );
          expect(
            WindowsFirewallService.rulePrefix,
            equals('Solaris_Control_API_'),
          );
        },
      );

      test(
        'isRuleConfigured handles port parameter cleanly without throwing',
        () async {
      ...
      test(
        'ensureRuleAdded returns boolean result safely on current OS platform',
        () async {
      ...
      test(
        'removeAllSolarisRules completes without throwing unhandled exceptions',
        () async {
