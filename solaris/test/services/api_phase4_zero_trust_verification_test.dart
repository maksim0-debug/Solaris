...
void main() {
  ...
  group('Phase 4 - Zero-Trust Advanced Verification & Deep Status Filtering QA Test Suite', () {
    ...
    group('A. BDD Behavioral Scenarios — Status & Query Read Protection', () {
      test('Scenario 1: Full Access configuration exposes all root modules in GET /api/v1/status', () async {
        updatePermissions(const ApiPermissionsConfig()); // Default: full access
      ...
      test('Scenario 2: GET /api/v1/health remains 100% public under total Lockdown mode', () async {
      ...
      test('Scenario 3: Granular sub-key masking in automation block when monitors & weather disabled', () async {
        updatePermissions(const ApiPermissionsConfig(
          allowReadMonitors: false,
          allowReadWeather: false,
      ...
      test('Scenario 4: Smart Circadian metrics stripping when allowReadSleep is false', () async {
        updatePermissions(const ApiPermissionsConfig(
          allowReadCircadian: true,
          allowReadSleep: false,
        ));
      ...
      test('Scenario 5: RFC 7807 Header & Payload Validation on Forbidden Read Endpoint', () async {
        ...
        expect(resp.headers.value('content-type'), contains('application/problem+json'));
    ...
    group('B. 32-Permutation Combinatorial Grid Test for GET /api/v1/status', () {
      test('Evaluates all 32 combinations of read flags to ensure zero data leakage', () async {
            ...
            for (final rWeather in flags) {
              for (final rSleep in flags) {
                for (final rCircadian in flags) {
                  ...
                  expect(body.containsKey('monitors'), equals(rMonitors),
                      reason: 'monitors presence must match allowReadMonitors');
                  expect(body.containsKey('solar'), equals(rSolar),
                      reason: 'solar presence must match allowReadSolar');
                  expect(body.containsKey('weather'), equals(rWeather),
                      reason: 'weather presence must match allowReadWeather');
                  expect(body.containsKey('sleep'), equals(rSleep),
                      reason: 'sleep presence must match allowReadSleep');
                  expect(body.containsKey('smart_circadian'), equals(rCircadian),
                      reason: 'smart_circadian presence must match allowReadCircadian');
    ...
    group('C. Robustness & Security Audit', () {
      test('GET /api/v1/presets returns brightness section when allowReadMonitors is true and allowReadCircadian is false', () async {
        updatePermissions(const ApiPermissionsConfig(
