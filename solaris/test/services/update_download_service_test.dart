...
void main() {
  ...
  group('UpdateDownloadService Tests', () {
    test(
      'verifyFileIntegrity returns true for matching SHA-256 digest',
      () async {
        ...
        final expectedDigestHex = sha256
            .convert(utf8.encode(fileContent))
            .toString();
    ...
    test(
      'verifyFileIntegrity returns false for mismatched SHA-256 digest',
      () async {
        final service = UpdateDownloadService();
    ...
    test(
      'downloadUpdate handles HTTP redirect and downloads content correctly',
      () async {
        final dummyPackageData = utf8.encode(
          'Solaris 1.0.18 Zip Package Payload',
        );
        ...
        final mockClient = MockClient((request) async {
          if (request.url.toString() ==
            ...
            expect(
              request.headers.containsKey('X-GitHub-Api-Version'),
              isFalse,
            );
            return http.Response(
              ...
              headers: {
                'location':
                    'https://objects.githubusercontent.com/download/Solaris-Windows.zip',
              },
            );
      ...
      final mockClient = MockClient((request) async {
        if (request.url.toString() ==
          ...
          return http.Response(
            ...
            headers: {
              'location':
                  'http://objects.githubusercontent.com/download/Solaris-Windows.zip',
            },
          );
        }
        return http.Response('Not Found', 404);
      ...
      final mockClient = MockClient((request) async {
        if (request.url.toString() ==
          ...
          return http.Response(
            ...
            headers: {
              'location':
                  'https://malicious-domain.com/download/Solaris-Windows.zip',
            },
          );
        }
        return http.Response('Not Found', 404);
    ...
    test('downloadUpdate supports HTTP 303 and 308 redirects', () async {
      final dummyPackageData = utf8.encode(
        'Solaris 1.0.18 Redirect 308 Payload',
      );
    ...
    test(
      'finalizeDownload promotes .tmp file to .zip and getCachedUpdate locates it',
      () async {
        final service = UpdateDownloadService();
        ...
        final finalPath = await service.finalizeDownload(
          tempFile.path,
          version,
        );
