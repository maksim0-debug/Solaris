...
void main() {
  ...
  group('Per-Monitor Settings & Selection Provider Tests', () {
    ...
    test(
      'SelectedMonitorsNotifier prevents empty selection and defaults to all',
      () {
    ...
    test(
      'SettingsNotifier updates specific monitor without crashing on new key',
      () async {
    ...
    test(
      'Selecting ALL MONITORS syncs per-monitor preset back to global preset',
      () async {
    ...
    test(
      'curvePoints fallback returns curvesMap when activeUserPresetId is invalid',
      () async {
        await container.read(settingsProvider.future);
    ...
    test(
      'Toggling auto-brightness off preserves user manual brightness setting',
      () async {
        await container.read(settingsProvider.future);
