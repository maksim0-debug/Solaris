...
void main() {
  ...
  group('LocationResolutionStatus Provider Tests', () {
    test(
      'resolves to manual when useManual is true and coordinates are set',
      () async {
    ...
    test(
      'resolves to autoSuccess when useManual is false and stream emits position',
      () async {
        ...
        final container = ProviderContainer(
          overrides: [
            ...
            locationStreamProvider.overrideWith((ref) => Stream.value(mockPos)),
          ],
        );
        addTearDown(container.dispose);
    ...
    test(
      'resolves to autoFailedTimezone when useManual is true but coordinates are null',
      () async {
        final container = ProviderContainer(
          overrides: [
            locationSettingsProvider.overrideWith(
    ...
    test(
      'resolves to autoFailedTimezone when useManual is false and stream is empty',
      () async {
        final container = ProviderContainer(
          overrides: [
            locationSettingsProvider.overrideWith(
    ...
    test(
      'getTimezoneFallbackCoordinates returns representative coordinates for major timezones',
      () {
    ...
    test(
      'currentWeatherProvider returns null when coordinates are unavailable',
      () async {
        final container = ProviderContainer(
          overrides: [
            locationSettingsProvider.overrideWith(
