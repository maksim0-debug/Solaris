...
void main() {
  ...
  group('Auto Update Toggle & Setting Tests', () {
    ...
    test('SettingsState serialization and copyWith handle isAutoUpdateEnabled', () {
    ...
    test('Automatic update check is skipped when isAutoUpdateEnabled is false', () async {
      ...
      container.read(settingsProvider.notifier).updateAutoUpdateEnabled(false);

      // Perform non-manual check
      await container.read(updateProvider.notifier).checkForUpdate(isManual: false);
    ...
    test('Manual update check proceeds even when isAutoUpdateEnabled is false', () async {
      final mockGithub = CountingMockGitHubService(sampleUpdateInfo);
      final container = ProviderContainer(
        overrides: [
          githubReleaseServiceProvider.overrideWithValue(mockGithub),
      ...
      await container.read(updateProvider.notifier).checkForUpdate(isManual: true);

      // State should have updated and mock API called
      expect(mockGithub.checkCount, equals(1));
      expect(container.read(updateProvider).phase, equals(UpdatePhase.available));
    });
  });
}
