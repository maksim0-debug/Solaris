...
void main() {
  group('KeyObfuscator (DPAPI & Legacy XOR) Tests', () {
    ...
    test('XOR Legacy Decryption throws DpapiInvalidDataException on corrupted base64', () {
      expect(
    ...
    if (Platform.isWindows) {
      ...
      test('Windows DPAPI Decrypt throws DpapiInvalidDataException on corrupted DPAPI data', () {
        expect(
          () => KeyObfuscator.decrypt('dpapi:U29tZSBpbnZhbGlkIGJhc2U2NCBkYXRh'), // valid base64 but invalid DPAPI blob
          throwsA(isA<DpapiInvalidDataException>()),
        );
      });

      test('Stealer attack simulation (CryptUnprotectData without proper entropy fails)', () {
        final encrypted = KeyObfuscator.encrypt(rawSecret);
        expect(encrypted.startsWith('dpapi:'), isTrue);
        ...
        final cryptUnprotectData = _crypt32.lookupFunction<
            Int32 Function(
        ...
        inputPointer.asTypedList(encryptedBytes.length).setAll(0, encryptedBytes);
        ...
        try {
          ...
          expect(result, equals(0)); // Should fail because we encrypted with custom entropy
      ...
      test('Non-Windows platforms fallback to XOR encryption with obf: prefix', () {
        final encrypted = KeyObfuscator.encrypt(rawSecret);
      ...
      test('Non-Windows platforms throw DpapiGenericException when trying to decrypt dpapi: prefixed data', () {
        expect(
