  ...
  group('Zero-Trust Phase 1 Verification Suite', () {
    ...
    group('BDD Scenarios: Granular Permission Rules & Aliases', () {
      test('Given an API Key with allowedActions set to only set_brightness, '
        ...
        expect(config.isActionAllowed('set_monitor_brightness'), isTrue,
      ...
      test('Given an API Key with allowedActions containing set_brightness_preset, '
        ...
        for (final alias in aliases) {
          expect(config.isActionAllowed(alias), isTrue,
        ...
        for (final alias in tempAliases) {
          expect(config.isActionAllowed(alias), isFalse,
              reason: 'Alias "$alias" must not be allowed when set_temperature_preset is missing');
        }
      });
      ...
      test('Given an API Key with allowedActions set to clear_failed_webhooks, '
        ...
        // Given
        final canonicalFromAlias = ApiPermissionsConfig.getCanonicalAction('clear_failed_webhooks');
        ...
        expect(ApiPermissionsConfig.getCategoryForAction('clear_failed_webhooks'),
            equals(ApiActionCategory.system));
      });

      test('Given an API Key in Read-Only mode, '
        ...
        for (final action in allCanonical) {
          expect(config.isActionAllowed(action), isFalse,
              reason: 'Read-only mode must override allowedActions for "$action"');
        }
      });
    });
    ...
    group('Property-Based Testing (PBT): Randomized Subsets & Invariants', () {
      ...
      final allCanonicalList = ApiPermissionsConfig.getAllCanonicalActions().toList();

      test('PBT Invariant: For any subset of actions S, isActionAllowed(a) is true iff canonical(a) in S', () {
        for (var i = 0; i < 100; i++) {
          // Shuffle and pick a random subset size between 1 and 24
          final subsetSize = random.nextInt(24) + 1;
          final shuffled = List<String>.from(allCanonicalList)..shuffle(random);
          ...
          for (final canonical in allCanonicalList) {
            ...
            expect(
              ...
              equals(expected),
              reason: 'PBT Iteration $i failed for action $canonical with subset $selectedSubset',
            );
          }
        }
      });

      test('PBT Invariant: Roundtrip JSON serialization maintains equality and null reset invariant', () {
        for (var i = 0; i < 100; i++) {
          final subsetSize = random.nextInt(25); // 0 to 24 actions
          final shuffled = List<String>.from(allCanonicalList)..shuffle(random);
          final selectedSubset = subsetSize == 0 ? <String>{} : shuffled.take(subsetSize).toSet();
          ...
          expect(restored, equals(original), reason: 'PBT Roundtrip failed on iteration $i');
    ...
    group('Edge Cases & Zero-Trust Malformed JSON Payload Defense', () {
      test('fromJson robustly handles malformed non-list and non-string values without throwing', () {
        final malformedPayloads = [
          {'allowedCategories': 12345, 'allowedActions': 'not_a_list'},
          {'allowedCategories': true, 'allowedActions': 99.99},
          {'allowedCategories': {'key': 'val'}, 'allowedActions': {'nested': 'object'}},
          {
            'allowedCategories': ['monitors', 999, null, 'invalid_cat'],
            'allowedActions': ['set_brightness', 123, null, true, 'non_existent_action_xyz']
          },
        ];

        for (final payload in malformedPayloads) {
          expect(() => ApiPermissionsConfig.fromJson(payload), returnsNormally);
      ...
      test('toJson sanitizes orphaned actions from disabled categories', () {
        ...
        if (json.containsKey('allowedActions')) {
          final actionsInJson = (json['allowedActions'] as List<dynamic>).cast<String>();
          expect(actionsInJson, contains('set_brightness_preset'));
          expect(actionsInJson, isNot(contains('set_brightness')));
        }
      });

      test('Clean Storage Protocol: 25 canonical actions automatically serialize to null allowedActions', () {
        ...
        expect(json.containsKey('allowedActions'), isFalse,
            reason: 'JSON must omit allowedActions key when all 25 actions are enabled');
