...
void main() {
  group('ApiKeyEntry Unit Tests', () {
    test(
      'generateSecureToken produces sol_sec_ prefix with 64 hex characters',
      () {
    ...
    test(
      'ApiKeyEntry.create initializes with generated ID, secure token, and defaults',
      () {
    ...
    test(
      'ApiKeyEntry.create falls back to default name when name is empty',
      () {
    ...
    test(
      'toJson and fromJson serialize and deserialize with KeyObfuscator token encryption',
      () {
        ...
        final original = ApiKeyEntry(
          ...
          name: 'Stream Deck',
          token:
              'sol_sec_abcdef1234567890abcdef1234567890abcdef1234567890abcdef1234567890',
    ...
    test(
      'ApiKeyEntry.fromJson gracefully recovers from invalid/corrupted token payload',
      () {
        final badJson = {
          'id': 'corrupted_key_1',
          'name': 'Broken Key',
        ...
        expect(
          restored.token.startsWith('sol_sec_'),
          isTrue,
        ); // Generated fallback
    ...
    test(
      'copyWith updates specified properties while preserving unassigned ones',
      () {
  ...
  group('SettingsState Multi-Key Migration & Serialization Tests', () {
    test('Default SettingsState contains 1 Default Key in apiKeys list', () {
      ...
      expect(
        settings.apiPermissions,
        equals(settings.apiKeys.first.permissions),
      );
      expect(settings.requireLocalToken, isFalse);
    });

    test(
      'SettingsState.fromJson migrates legacy apiAccessToken and apiPermissions',
      () {
        final rawLegacyToken =
            'sol_sec_legacy_token_12345678901234567890123456789012345678901234567890';
    ...
    test(
      'SettingsState.copyWith updates apiKeys and supports legacy apiAccessToken/apiPermissions overrides',
      () {
        final initial = SettingsState();
        final updatedPermissions = const ApiPermissionsConfig(isReadOnly: true);
        final newLegacyToken =
            'sol_sec_new_override_token_12345678901234567890123456789012345678901234';
