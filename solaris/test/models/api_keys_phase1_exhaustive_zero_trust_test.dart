...
void main() {
  group('Phase 1 - Zero-Trust Exhaustive Audit Suite: ApiKeyEntry & Data Models', () {
    group('A. BDD Behavioral Specifications (Given / When / Then)', () {
      test(
        'Scenario 1: Given a new ApiKeyEntry, When created with custom name, Then name is trimmed and secure token is generated',
        () {
      ...
      test(
        'Scenario 2: Given corrupted DPAPI token in JSON, When ApiKeyEntry.fromJson is called, Then graceful fallback triggers with isDpapiFallback=true',
        () {
          final corruptedJson = {
            'id': 'test_corrupt_id',
            'name': 'Corrupted Key',
            'token':
                'dpapi:corrupted_base64_payload_that_cannot_be_decrypted!!!',
      ...
      test(
        'Scenario 3: Given legacy SettingsState JSON without apiKeys, When deserialized, Then automatic migration creates Default Key with legacy token',
        () {
          final rawToken =
              'sol_sec_11223344556677889900aabbccddeeff11223344556677889900aabbccddeeff';
      ...
      test(
        'Scenario 4: Given empty apiKeys list in JSON, When SettingsState.fromJson parses, Then starter key is created to enforce non-empty invariant',
        () {
    ...
    group('B. Edge Cases & Defensive Realism Audit', () {
      test(
        '1. Malformed fields in ApiKeyEntry.fromJson do not cause unhandled crashes',
        () {
      ...
      test(
        '2. SettingsState.fromJson handles mixed invalid elements in apiKeys list gracefully',
        () {
          ...
          expect(settings.apiKeys.isNotEmpty, isTrue);
          final found = settings.apiKeys
              .where((k) => k.name == 'Valid Key')
              .toList();
      ...
      test(
        '3. SettingsState.apiKeys list is strictly unmodifiable (Immutability Check)',
        () {
          final settings = SettingsState();
          expect(
            () => (settings.apiKeys as List).add(
              ApiKeyEntry.create(name: 'Hack'),
            ),
      ...
      test(
        '4. Zero Secret Leakage: ApiKeyEntry.toJson encrypts token so raw secret never appears in plain text',
        () {
      ...
      test(
        '5. copyWith supports both multi-key updating and legacy parameter overrides',
        () {
          ...
          final updatedLegacy = initial.copyWith(
            apiAccessToken:
                'sol_sec_override_token_1234567890123456789012345678901234567890123456',
          );

          expect(updatedLegacy.apiKeys.length, equals(2));
          expect(
            updatedLegacy.apiKeys.first.token,
            equals(
              'sol_sec_override_token_1234567890123456789012345678901234567890123456',
            ),
          );
    ...
    group('C. Property-Based Testing (PBT) & Invariants Audit', () {
      test(
        'PBT Invariant 1: 100 generated tokens strictly follow sol_sec_[0-9a-fA-F]{64} format',
        () {
          ...
          for (int i = 0; i < 100; i++) {
            ...
            expect(
              token.length,
              equals(72),
              reason: 'Iteration $i failed length check',
            );
            expect(
              hexRegex.hasMatch(token),
              isTrue,
              reason: 'Iteration $i failed regex check',
            );
          }
        },
      );
      ...
      test(
        'PBT Invariant 2: SettingsState.apiKeys is NEVER empty across 50 random serialization roundtrips',
        () {
          ...
          for (int i = 0; i < 50; i++) {
            ...
            final keys = List.generate(
              keysCount,
              (idx) => ApiKeyEntry.create(name: 'Key $idx'),
            );
            ...
            expect(
              restored.apiKeys.isNotEmpty,
              isTrue,
              reason: 'Iteration $i produced empty apiKeys',
            );
      ...
      test(
        'PBT Invariant 3: Serialization/Deserialization Roundtrip Equality for ApiKeyEntry',
        () {
          ...
          final entry = ApiKeyEntry(
            ...
            token:
                'sol_sec_00112233445566778899aabbccddeeff00112233445566778899aabbccddeeff',
