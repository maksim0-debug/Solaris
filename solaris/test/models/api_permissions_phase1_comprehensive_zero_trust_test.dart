...
void main() {
  group('Phase 1: Zero-Trust Comprehensive BDD & Security Audit Suite', () {
    ...
    group('BDD Specifications', () {
      test(
        'Given default ApiPermissionsConfig, When initialized, Then full access is granted',
        () {
          // Given
          ...
          expect(
            ...
            containsAll(ApiActionCategory.values),
          );
        },
      );

      test(
        'Given JSON missing apiPermissions key, When deserializing SettingsState, Then fallback to full access',
        () {
          // Given
          final rawJson = <String, dynamic>{'localIpcServerPort': 8080};
      ...
      test(
        'Given isReadOnly = true, When checking any action, Then isActionAllowed returns false',
        () {
          // Given
          ...
          for (final action in actionsToTest) {
            expect(
              ...
              isFalse,
              reason: 'Action $action must be prohibited in Read-Only mode',
            );
          }
        },
      );
      ...
      test(
        'Given specific category disabled, When evaluating isActionAllowed, Then return false for that category only',
        () {
          // Given
          const config = ApiPermissionsConfig(
            allowedCategories: {
      ...
      test(
        'Given allowReadWeather = false, When filtering automation map, Then weather_* keys are completely removed',
        () {
          // Given
          ...
          // Then
          expect(
            filtered.containsKey('weather_brightness_adjustment'),
            isFalse,
          );
          expect(
            filtered.containsKey('weather_temperature_adjustment'),
            isFalse,
          );
      ...
      test(
        'Given allowReadCircadian = false, When filtering automation map, Then circadian & smart_circadian keys are removed',
        () {
          // Given
      ...
      test(
        'Given allowReadCircadian = true AND allowReadSleep = false, When filtering smart_circadian, Then sleep pressure & debt are stripped',
        () {
          // Given
          const config = ApiPermissionsConfig(
      ...
      test(
        'Given both allowReadMonitors and allowReadCircadian false, When filtering presets, Then return null',
        () {
          // Given
          const config = ApiPermissionsConfig(
    ...
    group('Exhaustive Action Category Mapping Audit', () {
      ...
      test('Verifies mapping for all 34 specified actions and aliases', () {
        expectedMappings.forEach((action, expectedCategory) {
          ...
          expect(
            ...
            reason: 'Action "$action" must map to ${expectedCategory.name}',
          );
        });
      });
      ...
      test('Verifies unrecognized actions map to null', () {
        ...
        for (final invalidAction in invalidActions) {
          expect(
            ApiPermissionsConfig.getCategoryForAction(invalidAction),
            isNull,
          );
        }
      });
    ...
    group('32-Combination Combinatorial Read Flag Grid Test', () {
      test(
        'Evaluates all 32 boolean permutations of 5 read flags for filterAutomation',
        () {
          ...
          for (int i = 0; i < 32; i++) {
            ...
            if (!allowWeather) {
              expect(
                filtered.keys.any((k) => k.startsWith('weather_')),
                isFalse,
            ...
            if (!allowCircadian) {
              expect(
                filtered.containsKey('smart_circadian'),
                isFalse,
                reason: 'Iteration $i: smart_circadian must be stripped',
              );
              expect(
                filtered.keys.any((k) => k.startsWith('circadian_')),
                isFalse,
            ...
            if (!allowCircadian && !allowMonitors) {
              ...
              expect(
                filtered.containsKey('color_temperature_hardware_enabled'),
                isFalse,
              );
            }
    ...
    group('Property-Based Testing (PBT) Invariants', () {
      ...
      test(
        'PBT Invariant 1: No weather keys survive when allowReadWeather is false',
        () {
          for (int run = 0; run < 100; run++) {
            ...
            final hasWeatherKeys = filtered.keys.any(
              (k) => k.startsWith('weather_'),
            );
            expect(
              hasWeatherKeys,
              isFalse,
              reason: 'Run $run: Invariant violated!',
            );
          }
        },
      );
      ...
      test(
        'PBT Invariant 2: No sleep metrics survive in smart_circadian when allowReadSleep is false',
        () {
          for (int run = 0; run < 100; run++) {
            final config = ApiPermissionsConfig(
              allowReadCircadian: true,
      ...
      test(
        'PBT Invariant 3: Serialization/Deserialization Roundtrip Identical Equality',
        () {
          for (int run = 0; run < 100; run++) {
    ...
    group('Edge-Case & Robustness Audit', () {
      test('Handles empty maps gracefully without exceptions', () {
        ...
        final filteredAutomation = ApiPermissionsFilter.filterAutomation(
          {},
          config,
        );
      ...
      test(
        'Handles malformed submodules type in smart_circadian without crashing',
        () {
          const config = ApiPermissionsConfig(
            allowReadCircadian: true,
            allowReadSleep: false,
          ...
          final malformedCircadian = <String, dynamic>{
            ...
            'sleep_pressure': 0.9,
            'submodules':
                'invalid_string_instead_of_map', // Edge case malformed JSON
          };
          ...
          expect(
            filtered['submodules'],
            equals('invalid_string_instead_of_map'),
          );
        },
      );
      ...
      test(
        'ApiPermissionsCategory.fromJson handles unknown string gracefully by defaulting to system',
        () {
          final category = ApiActionCategory.fromJson(
            'non_existent_category_name',
          );
