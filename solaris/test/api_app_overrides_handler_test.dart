...
void main() {
  ...
  group('ApiAppOverridesHandler Phase 4 Zero-Trust Integration & ACL Tests', () {
    ...
    test('GET /api/v1/app-overrides returns default 6 built-in rules and exit_delay_seconds', () async {
      final req = await client.getUrl(Uri.parse('$serverUrl/api/v1/app-overrides'));
    ...
    test('GET /api/v1/app-overrides/active returns current active process and applied rule', () async {
      container.read(activeProcessServiceProvider.notifier).updateActiveProcessManually(
        'photoshop.exe',
        title: 'Adobe Photoshop 2026',
      );

      final req = await client.getUrl(Uri.parse('$serverUrl/api/v1/app-overrides/active'));
      final resp = await req.close();
    ...
    test('POST /api/v1/app-overrides creates a new custom rule and updates settingsProvider', () async {
      ...
      final req = await client.postUrl(Uri.parse('$serverUrl/api/v1/app-overrides'));
      ...
      final added = settings!.appOverrides.firstWhere((r) => r.exeName == 'code.exe');
      expect(added.fixedTemperature, equals(5000.0));
    });

    test('POST /api/v1/app-overrides rejects path traversal attempt with RFC 7807 400 Bad Request', () async {
      ...
      expect(json['type'], equals('https://solaris.local/errors/app-overrides-error'));
      expect(json['title'], equals('Validation Error'));
      expect(json['detail'], contains('path traversal characters are strictly prohibited'));
    });

    test('POST /api/v1/app-overrides rejects non-.exe extensions with RFC 7807 400 Bad Request', () async {
      ...
      expect(json['title'], equals('Validation Error'));
      expect(json['detail'], contains('must be a valid executable filename ending in .exe'));
    });

    test('POST /api/v1/app-overrides rejects empty body with RFC 7807 400 Bad Request', () async {
      final req = await client.postUrl(Uri.parse('$serverUrl/api/v1/app-overrides'));
      req.headers.contentType = ContentType.json;
      req.write('   ');
      final resp = await req.close();
    ...
    test('DELETE /api/v1/app-overrides/:exe removes rule successfully', () async {
      container.read(settingsProvider.notifier).addAppOverride(
      ...
      final req = await client.deleteUrl(Uri.parse('$serverUrl/api/v1/app-overrides/blender.exe'));
      final resp = await req.close();
      ...
      final settings = container.read(settingsProvider).value?['all'];
      final exists = settings!.appOverrides.any((r) => r.exeName == 'blender.exe');
      expect(exists, isFalse);
    });

    test('DELETE /api/v1/app-overrides/:exe with path traversal returns 400 Bad Request', () async {
      final req = await client.deleteUrl(Uri.parse('$serverUrl/api/v1/app-overrides/..%2Fcmd.exe'));
      final resp = await req.close();

      expect(resp.statusCode, equals(HttpStatus.badRequest));
    });

    test('POST /api/v1/app-overrides/reset-builtin resets built-in rules to factory defaults', () async {
      container.read(settingsProvider.notifier).removeAppOverride('photoshop.exe');

      final req = await client.postUrl(Uri.parse('$serverUrl/api/v1/app-overrides/reset-builtin'));
      final resp = await req.close();

      expect(resp.statusCode, equals(HttpStatus.ok));

      final settings = container.read(settingsProvider).value?['all'];
      final psRule = settings!.appOverrides.firstWhere((r) => r.exeName == 'photoshop.exe');
      expect(psRule.isBuiltIn, isTrue);
      expect(psRule.fixedTemperature, equals(6500.0));
    });

    test('ACL Permission enforcement: Read-only token returns 403 Forbidden RFC 7807 on POST', () async {
      const readOnlyPermissions = ApiPermissionsConfig(
        isReadOnly: true,
      );
      container.read(settingsProvider.notifier).updateApiPermissions(readOnlyPermissions);
      ...
      expect(json['type'], equals('https://solaris.local/errors/access-denied'));
      expect(json['title'], equals('Read-Only Mode Enabled'));
    });

    test('ACL Permission enforcement: Restricting allowedActions returns 403 Forbidden when action is absent', () async {
      // Allow only get_app_overrides
      const restrictedPermissions = ApiPermissionsConfig(
        allowedActions: {'get_app_overrides'},
      );
      container.read(settingsProvider.notifier).updateApiPermissions(restrictedPermissions);
      ...
      expect(json['detail'], contains('disabled in API permissions settings'));
    });
  });
}
