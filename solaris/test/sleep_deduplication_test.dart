...
void main() {
  ...
  group('Sleep Sessions Deduplication Tests', () {
    ...
    test('Adding local_api sessions works and orders them by startTime descending', () async {
    ...
    test('Local API session overwrites/removes overlapping Google Fit session', () async {
      final notifier = container.read(sleepProvider.notifier);
    ...
    test('Google Fit session is discarded if it overlaps with an existing Local API session', () async {
      final notifier = container.read(sleepProvider.notifier);
