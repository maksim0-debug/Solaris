...
void main() {
  group('Native Process Detection & EventChannel Contract Tests', () {
    ...
    test(
      'EventChannel map payload handles non-ASCII / UTF-8 process names',
      () {
        final rawPayload = <Object?, Object?>{
          'is_gaming': false,
    ...
    test('MethodChannel getRunningProcesses payload format parsing', () {
      ...
      final parsedProcesses = rawList.map((item) {
        ...
        return {'exe': map['exe'] as String, 'title': map['title'] as String};
      }).toList();
    ...
    test(
      'MethodChannel getRunningProcesses excludes self process solaris.exe',
      () {
        final processes = [
    ...
    test('Process name lowercasing and normalization contract', () {
      ...
      expect(
        normalizeExe(r'C:\Program Files\Adobe\Photoshop.exe'),
        equals('photoshop.exe'),
      );
      expect(normalizeExe(r'C:\ПОЛЬЗОВАТЕЛИ\ТЕСТ\APP.EXE'), equals('app.exe'));
      expect(
        normalizeExe('ApplicationFrameHost.exe'),
        equals('applicationframehost.exe'),
      );
    });
  });
}
