...
void main() {
  ...
  group('Stage 4 Zero-Trust GUI Verification: ApiPermissionsDialog & Granular Tri-State UI', () {
    testWidgets('BDD Scenario 1: Toggling single action triggers Tri-State [-] and returns 24 actions', (WidgetTester tester) async {
      ...
      final monitorsHeader = find.text('Monitors (Brightness, Temp, Offsets)');
      ...
      expect(resultConfig!.allowedActions!.contains('set_temperature'), isFalse);
    });

    testWidgets('BDD Scenario 2: Clean Storage Protocol resets allowedActions to null when all 25 selected', (WidgetTester tester) async {
      tester.view.physicalSize = const Size(1200, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      addTearDown(tester.view.resetDevicePixelRatio);
    ...
    testWidgets('BDD Scenario 3: Unchecking category checkbox disables category and strips actions', (WidgetTester tester) async {
      tester.view.physicalSize = const Size(1200, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      addTearDown(tester.view.resetDevicePixelRatio);
      ...
      final sleepCheckbox = find.descendant(
        of: find.ancestor(of: sleepTile, matching: find.byType(ExpansionTile)),
      ...
      expect(resultConfig, isNotNull);
      expect(resultConfig!.allowedCategories.contains(ApiActionCategory.sleep), isFalse);
    });

    testWidgets('BDD Scenario 4: Multilingual locale rendering (Russian & Ukrainian)', (WidgetTester tester) async {
      tester.view.physicalSize = const Size(1200, 1200);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      addTearDown(tester.view.resetDevicePixelRatio);
      ...
      final ruHeader = find.text('Мониторы (яркость, температура, смещения)');
