...
void main() {
  group('CancelableNetworkTileProvider Unit Tests', () {
    test('getImage returns _CancelableNetworkImageProvider with custom httpClient', () {
      ...
      final options = TileLayer(urlTemplate: 'https://tile.openstreetmap.org/{z}/{x}/{y}.png');

      final imageProvider = provider.getImage(coords, options);

      expect(imageProvider, isNotNull);
      expect(imageProvider.runtimeType.toString(), contains('CancelableNetworkImageProvider'));
    ...
    test('dispose preserves external httpClient when ownsHttpClient is false', () {
      bool isClosed = false;
      final client = _TrackingClient(() => isClosed = true);
      final provider = CancelableNetworkTileProvider(
        httpClient: client,
