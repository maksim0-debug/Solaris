...
void main() {
  ...
  group('Phase 3 - Zero-Trust Exhaustive UI & Management Interface Test Suite', () {
    ...
    // -------------------------------------------------------------------------
    testWidgets('Scenario 1: Single Key Protection Guard prevents deleting the sole remaining API key in GUI and Notifier', (WidgetTester tester) async {
    ...
    testWidgets('Scenario 2: Adding a second key enables deletion for non-sole keys', (WidgetTester tester) async {
      configureScreenSize(tester);
    ...
    // -------------------------------------------------------------------------
    testWidgets('Scenario 3: CreateApiKeyDialog trims whitespace, enforces length limit and defaults name', (WidgetTester tester) async {
      configureScreenSize(tester);
            ...
            home: Scaffold(
              body: Builder(
                builder: (context) => ElevatedButton(
                  onPressed: () async {
                    createdEntry = await showCreateApiKeyDialog(context, existingKeysCount: 2);
                  },
      ...
      await tester.enterText(find.byType(TextFormField), '   Home Automation Key   ');
      await tester.pump();
      ...
      expect(createdEntry!.token.length, equals(72)); // sol_sec_ (8) + 64 hex chars
    });

    // -------------------------------------------------------------------------
    // C. DPAPI Fallback Warning Banner Non-Auto-Hiding Lifetime Audit
    // -------------------------------------------------------------------------
    testWidgets('Scenario 4: DPAPI Fallback Warning Banner persists until explicitly dismissed via X button', (WidgetTester tester) async {
      configureScreenSize(tester);
    ...
    // -------------------------------------------------------------------------
    testWidgets('Scenario 5: ShowNewTokenDialog copies token to Clipboard and dismisses safely', (WidgetTester tester) async {
      configureScreenSize(tester);
      ...
      tester.binding.defaultBinaryMessenger.setMockMethodCallHandler(
        ...
        (MethodCall methodCall) async {
          if (methodCall.method == 'Clipboard.setData') {
            final Map<String, dynamic> args = methodCall.arguments as Map<String, dynamic>;
      ...
      final savedBtn = find.widgetWithText(ElevatedButton, 'I Have Saved the Token');
    ...
    // -------------------------------------------------------------------------
    testWidgets('Scenario 6: ApiPermissionsDialog does NOT mutate settingsProvider directly and pops updated config', (WidgetTester tester) async {
      configureScreenSize(tester);
                ...
                builder: (context) => ElevatedButton(
                  onPressed: () async {
                    returnedConfig = await showApiPermissionsDialog(
                      context,
                      initialConfig: const ApiPermissionsConfig(isReadOnly: false),
                    );
                  },
    ...
    // -------------------------------------------------------------------------
    testWidgets('Scenario 7: Multilingual UI completeness check across English, Russian, and Ukrainian locales', (WidgetTester tester) async {
      configureScreenSize(tester);

      final key = ApiKeyEntry.create(name: 'Test Key');
      final settings = SettingsState(isLocalIpcServerEnabled: true, apiKeys: [key]);
...
class TestSettingsNotifier extends SettingsNotifier {
  ...
  void addApiKey(ApiKeyEntry entry) {
    final updatedKeys = List<ApiKeyEntry>.from(_currentSettings.apiKeys)..add(entry);
  ...
  bool removeApiKey(String id) {
    ...
    final updatedKeys = _currentSettings.apiKeys.where((k) => k.id != id).toList();
    _currentSettings = _currentSettings.copyWith(apiKeys: updatedKeys);
    state = AsyncData({'all': _currentSettings});
    return true;
  }
